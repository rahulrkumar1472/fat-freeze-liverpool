...
function LinkList({ title, links }: { title: string; links: Array<{ label: string; href: string }> }) {
  return (
    <div>
      <h3 className="text-xs font-semibold uppercase tracking-[0.14em] !text-white/90">{title}</h3>
...
export function Footer() {
  return (
    <footer className="border-t border-[var(--border)] bg-[var(--accent-navy)] text-[var(--on-dark)]">
      <Container className="py-14">
        <div className="grid gap-10 xl:grid-cols-[1.15fr_1fr_1fr_1fr_1fr]">
          <div>
            <h2 className="font-display text-xl font-semibold !text-white">{siteConfig.clinicName}</h2>
            <p className="mt-4 text-sm leading-7 text-white/78">{siteConfig.address}</p>
            <p className="mt-1 text-sm leading-7 text-white/78">Phone: {siteConfig.phone}</p>
            <p className="mt-1 text-sm leading-7 text-white/78">Email: {siteConfig.enquiryEmail}</p>
            <p className="mt-1 text-sm leading-7 text-white/78">Opening hours: Monday-Sunday, 10:00-20:00</p>
            <ul className="mt-5 space-y-2 text-xs text-white/62">
        ...
        <div className="mt-10 rounded-2xl border border-[#26445e] bg-[#0d2238] p-5">
          ...
          <p className="mt-2 text-sm leading-8 text-white/78">
          ...
          <p className="mt-3 text-xs leading-6 text-white/60">{liverpoolDistrictSignals.join(" • ")}</p>
        </div>
      </Container>

      <div className="border-t border-[#26445e] px-4 py-6 text-center text-xs text-white/60">
